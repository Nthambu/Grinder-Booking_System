...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT reset_attempts, last_reset_attempt FROM users WHERE email = ?");
    ...
    if ($reset_attempts >= 4) {
        ...
        $reset_attempts++;
        $stmt = $conn->prepare("UPDATE users SET reset_token = ?, reset_token_expiry = ?, reset_attempts = ?, last_reset_attempt = NOW() WHERE email = ?");
        $stmt->bind_param("ssis", $token, $expiry, $reset_attempts, $email);
        if ($stmt->execute()) {
            // Construct the reset link
            $reset_link = "http://localhost/services/Reset.php?token=$token";
